...
#import "CDVIFrameScript.h"
#import <WebKit/WebKit.h>

@interface CDVIFrameScript () {}
@end

@implementation CDVIFrameScript
...
- (void) addScriptFile:(CDVInvokedUrlCommand*)command
{
    ...
    if ([self scriptNotLoaded:scriptId]) {
        ...
        if (source == nil || error != nil) {
            NSLog(@"CDVIFrameScript: Error reading script file: %@", error);
        } else {
...
- (bool) scriptNotLoaded:(NSString*)scriptId
{
    ...
    if (self.idsLoaded[scriptId]) {
        NSLog(@"CDVIFrameScript: Script already loaded, ignore: %@", scriptId);
